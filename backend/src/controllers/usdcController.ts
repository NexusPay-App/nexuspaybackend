...
import { Request, Response } from 'express';
import { ethers, providers } from 'ethers';
import { getProvider, getTokenAddress } from '../config/constants'; // Modify to import getProvider and getTokenAddress functions
...
export const getUsdcBalance = async (req: Request, res: Response) => {
  try {
    const address = req.params.address;
    const chain = req.params.chain; // Get chain parameter from request

    const provider = getProvider(chain); // Get provider based on the chain
    const tokenAddress = getTokenAddress(chain); // Get token address based on the chain
